  ...
  function renderSectionForm(section, label) {
    ...
    return `
      <section class="form-card editor-section-card" id="section-${esc(section)}" data-section-card="${esc(section)}" data-sortable-section data-panel="${esc(section)}">
        ...
        <div class="section-content" ${isExpanded ? "" : "hidden"}>
          ${items.length ? `<button class="open-all-pill" type="button" data-action="toggle-all-items" data-section="${esc(section)}">${isAllExpanded ? `Cierra todos los ${esc(copy.toggleLabel || copy.title)}` : `Abre todo ${esc(copy.toggleLabel || copy.title)}`}${renderOpenAllChevron(isAllExpanded)}</button>` : ""}
